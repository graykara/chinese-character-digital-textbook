          ...
          <div className="absolute top-[105px] w-[1460px]">
            ...
            <div className="grid grid-cols-[500px__1fr] gap-5 mt-[85px] ml-[35px]">
              ...
              <div>
                ...
                <p className="ml-[70px] mt-6 text-[40px] text-[#1b1b1b] tracking-tight break-keep leading-[55px] ">태종 이방원이 즉위한 후, 이성계가 함흥에 있을 때 자신을 한양으로 데려가기 위해 온 차사들을 죽이거나 가두어 돌려보내지 않은 것에서 유래하였다.</p>
                <p className="ml-[70px] mt-1 text-[30px] text-[#1b1b1b] tracking-tight break-keep leading-[55px] "><span className="text-[#86482e] text-[40px]">*</span> 차사: 임금이 중요한 임무를 위하여 파견하던 임시 벼슬.</p>
              </div>
            </div>
          </div>
        )}
        {step === 2 && (
          <div className="absolute top-[115px] w-[1460px] left-[50px] bg-transparent">
            <div className="grid grid-cols-[1fr__500px] items-center gap-8 ml-1">
              <div>
                <img src={IMAGE2.src} />
                <p className="ml-1 mt-11 text-[40px] text-[#1b1b1b] tracking-tight break-keep leading-[55px] ">세종 대왕이 황희(<span className="font-haeseo text-[40px] leading-tight">黃喜</span>)의 검소함을 안타깝게 여겨 하루 동안 숭례문으로 들어오는 물건을 모두 가져다주라고 하였다. 그러나 이날은 큰비가 와서 들어온 물건이 달걀 한 꾸러미뿐이었는데 이것 마저 모두 곯아서 먹을 수가 없었던 것에서 유래하였다.</p>
                <p className="ml-1 mt-1 text-[30px] text-[#1b1b1b] tracking-tight break-keep leading-[35px] "><span className="text-[#86482e] text-[40px]">*</span>  ‘골(<span className="font-haeseo text-[30px] leading-tight">骨</span>)’자는 ‘곯았다’의 ‘곯’에 해당하는 한자음이 없기 때문에 비슷한 발음의<br />
                  <span className="ml-[22px]">‘골’을 사용한 것이다. 따라서 본래 뜻(뼈)과 관계없이 쓰였다.</span></p>
              </div>
              <VideoThumbnail
