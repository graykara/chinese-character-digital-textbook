  ...
  return (
    <div className={`${className}`}>
      {options.map((option, index) => (
        ...
        >
          <div
            className={`w-14 h-14 rounded-full text-white flex justify-center items-center ${showAnswer && answers.includes(index + 1) ? "bg-answer" : !showAnswer && selectedOptions.includes(index + 1) ? "bg-example" : "bg-[#5c94ae]"}`}
          >
            {index + 1}
          </div>
          {option}
